...
"""
DEPRECATED -- imported by nothing, and it does not run: the plotting helpers use `plt` with no
matplotlib import, and MOPED_compression references a `constants` module that no longer exists.
Written for the Perlmutter GPU nodes at NERSC. Kept for reference only.
...
def moped_reduce(
    ...
    for all_fidu, all_vali, all_grid, take in zip(fidu_splits, vali_splits, grid_splits, all_take):
        ...
        bs = []
        b1 = inv_cov.dot(derivatives[zero_ind]) / np.sqrt(
            derivatives[zero_ind].dot(inv_cov.dot(derivatives[zero_ind]))
        )
        bs = [b1]
